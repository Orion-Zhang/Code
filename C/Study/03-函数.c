...
//	int c = Add(a, b);
//	printf("%d + %d = %d\n", a, b, c);
//	return 0;//"main"函数的返回值代表着程序是否正常退出，为零代表程序正常退出，非零代表程序异常退出。
//}
...
/**
	C语言中函数的分类
		1.库函数
			...
			c.库函数常分为七大类的函数：输入/输出函数("I/O"函数)、字符串操作函数、字符操作函数、内存操作函数、时间/日期函数、数学函数、其他库函数。
			...
			f.于"VS2022"中禁止使用部分不安全的库函数，如"scanf"函数，若要使用不安全函数(屏蔽警告)需在文件首行使用"#define _CRT_SECURE_NO_WARNINGS"。
		2.自定义函数
			a.库函数并不能满足大多数程序的全部功能，这样就需要自定义函数，自定义函数的形式与库函数相同，不同的是自定义函数是有程序员自己设计的。
			b.与库函数相同，自定义函数也可以单独放在".c"文件实现、放在".h"文件声明，但某些库函数的函数实现一般被封装成了静态库。
			c.若使用不同文件下的自定义函数，同样需要引用头文件，若头文件在源文件目录下需要使用双引号引用头文件。(符号区别详见程序环境和预处理章节)
			d.自定义函数中的任何部分都是可以由自己定义的。(须遵守相关规则)
*/
...
//}
//
//int main()//在C语言中"main"函数后的括号空着代表着编译器对是否接受参数保持沉默，若传递参数给"main"函数，编译器会允许此行为，但若在"main"函数后的括号加上"void"，则编译器报警告，注意不是报错。
//{
//	int num = 0;
//	printf("调用函数前：num = %d。\n", num);
//	Change(num);
...
////函数的调用练习题例二：写一个函数，实现一个整型有序数组的二分查找。
