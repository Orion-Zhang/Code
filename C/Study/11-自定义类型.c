...
//int main()
//{
//	struct Student x = { "Aoki",19,"2020412505XX" };//使用"struct Student"这个结构体类型创建出"x"这个变量。
//	return 0;
//}
...
//	char b;
//	float c;
//}*ptr;//使用此匿名结构体类型创建出一个"ptr"全局指针变量。
//
//int main()
//{
//	ptr = &x;//未定义行为：编译器会把上面的两个变量"ptr"和"x"当成两个完全不同的类型，故此行代码会使编译器报警告。
//	return 0;
//}

////结构体类型的声明与定义示例三：在一个结构体类型中包含另一个结构体类型变量。
//struct Privacy//声明一个结构体类型"struct Privacy"。
//{
//	char sexual_orientation[5];
...
//	int age;
//	char id[20];
//	struct Privacy privacy;//此结构体包含另一个结构体类型"struct Privacy"。
//};
//
//int main()
//{
//	struct Student x = { "Aoki",19,"2020412505XX",{"保密","186XXXXXXXX"} };
//	printf("%s %d %s %s %s\n", x.name, x.age, x.id, x.privacy.sexual_orientation, x.privacy.phone_number);
//	return 0;
//}

/***
	结构体的自引用：在结构体中包含一个类型为该结构体本身的成员。
		1.结构体中包含一个类型为该结构体本身的成员，即包含同类型的结构体指针变量，而不是包含同类型的结构体变量。
...
//{
//	int data;
//	struct Node* next;//自引用的正确方法。
//	//struct Node next;//自引用的错误方法：陷入死循环，无法计算此结构体类型的大小，取消此行注释编译器将报错。
//};
//
//int main()
//{
...
////结构体自引用示例二：利用"typedef"关键字将结构体类型重命名。
//typedef struct Node//使用"typedef"将"struct Node"重命名为"Node"。
//{
//	int data;
//	struct Node* next;
//	//pNode prev;//在程序执行至此行时，还未完成结构体类型的重命名，故编译器无法知道"pNode"是什么，导致取消此行注释编译器报错。
//}Node, * NodePtr;//将"struct Node"重命名为"Node"以及将"struct Node*"重命名为"NodePtr"。
//
//int main()
//{
//	Node x = { 0 };//等价于"struct Node x = { 0 };"。
//	NodePtr px = &x;//等价于"struct Node* px = &x;"。
//	return 0;
//}

////结构体自引用例题：解答以下代码中"const"关键词修饰的对象。
//typedef struct Node
//{
//	int data;
//	struct Node* next;
...
//	Node y = { 0 };
//
//	const NodePtr px = &x;//"const"修饰的是"px"这个指针变量，而不是这个指针变量指向的"x"，即此行等价于"struct Node * const px = &x;"。
//
//	NodePtr const py = &y;//"const"修饰的是"py"这个指针变量。
//
//	return 0;
//}
...
//	int age;//年龄。
//	char id[20];//学号。
//	struct Privacy privacy;//隐私。
//}x, y = { 0 };//创建结构体对象的第一种方式：使用"struct Student"类型创建"x"和"y"变量，这两个变量为全局变量；"struct Student y = { 0 }"定义结构体变量并进行初始化(嵌套)。
//
//int main()
//{
//	struct Student z = { "Aoki",19,"2020412505XX",{"保密","186XXXXXXXX"} };//创建结构体对象的第二种方式：使用"struct Student"类型创建"z"变量并进行初始化，此变量为局部变量。
//	
//	struct Student* pz = &z;//将"z"的地址存放到结构体类型为"struct Student"的指针变量"pz"中。
//	
//	//结构体成员访问(使用"."操作符)
//	printf("%s\n", z.name);
//	printf("%d\n", (*pz).age);
...
//	printf("%s\n", pz->id);
//	printf("%s\n", pz->privacy.sexual_orientation);
//	printf("%s\n", pz->privacy.phone_number);
//
//	return 0;
//}
...
/***
	结构体内存对齐：拿空间换取时间。
		1.结构体内存对齐规则
			...
			c.在"VS2022"中默认对齐数为"8"。
			d.结构体的总大小为当前已存放的总内存大小扩展到结构体中每个成员的对齐数中的最大对齐数的整数倍。
		...
		4.修改默认对齐数：结构在对齐方式不合适的时候，我们可以自己更改默认对齐数。
			a.使用"#pragma pack()"预处理指令可修改默认对齐数，当"()"内无值则恢复默认对齐数，当"()"内有值则修改对齐数为对应对齐数。
		5."offsetof"宏的使用。
*/

////结构体内存对齐示例与例题：探讨如何计算结构体类型的大小(在64位平台下)。
//struct S1
//{
//	int x;
//	char y;
...
/****
	"offsetof"宏：用于求结构体中一个成员在该结构体中的偏移量。
		...
		3.未定义情况
			...
			c.C23标准起，若在"type"中定义新类型，则行为未定义，但"offsetof(struct Foo { int a; }, a)"通常支持，"offsetof(struct Foo { int a, b; }, a)"不支持，因为"struct Foo"定义中的逗号。
*/
...
/***
	结构体传参
		...
		2.若传递的结构体对象所占空间较大，那么参数压栈的系统开销就比较大，会导致性能的下降，故结构体传参时，建议传结构体的地址。
*/
...
//};
//
//void Print(struct Student x, struct Student* y)
//{
//	printf("\n%s %d %s %s %s\n", x.name, x.age, x.id, x.privacy.sexual_orientation, x.privacy.phone_number);
//	printf("\n%s %d %s %s %s\n", y->name, y->age, y->id, y->privacy.sexual_orientation, y->privacy.phone_number);
//}
//
//int main()
//{
...
//	struct Student* px = &x;
//
//	Print(x, px);//参数"x"是将结构体作为形参传递给了子函数；参数"px"是将此指针变量存放的结构体的地址传递给了子函数。
//
//	return 0;
//}
...
//int main()
//{
//	struct Bit_Field x = { 0 };
//	
//	x.a = 10;//十进制的"10"转换为二进制是"1010"，因位段中定义了"a"存储"3"个比特位，故会将"010"存放进存储单元。
//	x.b = 12;//十进制的"12"转换为二进制是"1100"，因位段中定义了"b"存储"4"个比特位，故会将"1100"存放进存储单元。
//	x.c = 3;//十进制的"3"转换为二进制是"11"，因位段中定义了"c"存储"5"个比特位，故会将"00011"存放进存储单元。
//	x.d = 4;//十进制的"4"转换为二进制是"100"，因位段中定义了"d"存储"4"个比特位，故会将"0100"存放进存储单元。
//	
...
//	//调试观察"x"在内存存放的数值可以发现三个字节存放的数据分别是"62"、"03"、"04"(十六进制)，假设成立。
//
//	return 0;
//}
...
/**
	枚举：对一个对象的所有可能取到的值的集合。
		1.枚举的声明与定义
			...
			b.枚举中每个成员(标识符)的取值称为枚举常量，默认从"0"开始，初始化时可以赋初始值，以后的标识符仍依次加"1"。
			c.枚举变量只能取枚举说明结构中的某个标识符常量。
		2.枚举的优点
			a.增加代码的可读性和可维护性。
			b.和"#define"宏定义的标识符相比较，枚举具有类型检查，但在C编译器中较难体现，在C++中较为直观(C++对枚举进行了扩充)，更加严谨。
			c.防止了命名污染(封装)。
			d.便于调试。
			e.使用方便，一次可以定义多个常量。
		3.枚举的使用
...
//	GREEN = 47,
//	BLUE = 167
//}r = RED;//创建枚举变量的第一种方式：使用"enum Color"类型创建"c"变量，此为全局变量；"enum Color c = RED"定义枚举变量并进行初始化(嵌套)。
//
//int main()
//{
...
//int main()
//{
//	union MyUnion un = { 5 };//调试发现变量"un"中的变量"x"与变量"y"的值都为"5"，说明联合体的成员是共用同一块内存空间的。
//
//	//因为联合体的成员是共用同一块内存空间的，所以以下输出的地址都相同。
//	printf("%p\n", &un);
//	printf("%p\n", &(un.x));
//	printf("%p\n", &(un.y));
//
//	un.x = 1000;
//	//调试发现当执行完代码"un.x = 1000;"后，"x"为"1000"、"y"为"-24"。
//	//十进制"1000"的二进制值为"00000000000000000000001111101000"。
//	//当以"un.x"赋值，即以"int"类型赋值，能赋值"32"位比特位的数据给联合体的存储单元，读取也是"32"位，故"x"为"1000"(十进制)。
//	//因为"y"为"char"类型，读取存储单元会发生截断，也就是只能读取二进制的"11101000"，其中最高位是符号位，转换为原码后得出结果为"-24"(十进制)。
//
//	un.y = 100;
//	//调试发现当执行完代码"un.y = 100;"后，"x"为"868"、"y"为"100"。
//	//十进制"100"的二进制值为"00000000000000000000000001100100"。
//	//当以"un.y"赋值，即以"char"类型赋值，只能赋值"8"位比特位的数据给联合体的存储单元，也就是赋值二进制"01100100"，故联合体的存储单元低八位被赋值，其余位不变。
...
//	//即以"int"类型的对齐数"4"作为最大对齐数，也就是扩展联合体大小到对齐数"4"的整数倍，故此联合体的大小为"8"字节。
//
//	return 0;
//}

////联合体的使用示例与例题：判断当前计算机的大小端存储。
//int Check_System()
//{
//	union
//	{
