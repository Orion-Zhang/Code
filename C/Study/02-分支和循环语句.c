...
/**
	"switch"语句：常用于多分支情况。
		...
		5."default"语句可以放在"switch"语句代码块中的任意位置，而且语句流会像执行一个"case"标签一样执行"default"子句。
...
/**
	"while"语句：需用重复执行多次的代码块常使用循环执行语句。
		...
		3."while(1)"表示死循环，表示若无跳出语句，则一直重复运行代码块内的语句，死循环可应用于单片机或特殊程序。
*/
...
/**
	"for"语句：包含初始化部分、条件判断部分、调整部分。
		1."for(;;)"可表示为死循环，这条语句省略了"for"语句的三个部分，导致恒为真。
		2."for"语句的执行顺序
...
/**
	布尔类型
		...
		2.布尔类型有且只有两个取值，即"true"和"false"，并且布尔类型的大小为"1"字节，另外布尔类型是一种无符号类型。
