	...
	移位操作符："<<"(左移)、">>"(右移)。
		...
		4.">>"(右移)：分为逻辑移位和算术移位。
			...
			d.对于数值的变化：除了负奇数外，当一个数右移"N"位时，相当于这个数除以"2"的"N"次方。(若数为奇数则结果默认向零取整)
				a'.当数为非负数时，右移一位等价于除以二。
				b'.当数为负数并且为偶数时，右移一位等价于除以二。
				c'.当数为负数并且为奇数时，负数的奇数除以二会比右移一位大一。
...
/**
	表达式
		...
		2.隐式类型转换
			a.整型提升
				a'.C语言的整型算术运算总是至少以缺省整型类型的精度来进行的。
				b'.为了获得精度，表达式中的字符类型操作数和短整型操作数在使用之前类型会被转换为普通整型，这种转换称为整型提升。
				c'.处理器内的"ALU"(整型运算器)的操作数的字节长度一般就是整型类型"int"的字节长度，同时也是处理器的通用寄存器的长度。
				d'.整型提升是按照变量的数据类型的符号位来提升的。
				e'.若是无符号数的整型提升，则为高位补"0"。
			b.算术转换
