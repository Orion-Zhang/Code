	...
	一维数组：一组用于存放相同类型元素的集合。
		...
		3.一维数组的初始化：在创建数组的同时给数组中的内容赋值，若在数组创建时不进行初识化，则"const_n"必须给出明确的值。
			a.完全初始化：将数组内的所有元素全进行赋值。
				a'.若在数组创建时进行初始化，则"const_n"可以省略，编译器将根据初始化的元素个数确定"const_n"，此为完全初始化。
			b.不完全初始化：将数组内的部分元素进行赋值。
				b'.不完成初始化剩余未被赋值的元素会被编译器进行自动初始化，但因不同编译器、不同操作系统、不同元素类型等原因造成自动初始化的值并不唯一。
		4.一维数组的使用
...
//
//	int arr2[10] = { 0 };
//	printf("%p\n", &arr2);//此时的数组名表示为整个数组，取出的是数组的地址，其依旧是首元素的地址，即整个数组的地址和首元素的地址的值相同，但类型不同。
