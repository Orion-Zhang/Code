#include <stdio.h>  //标准输入输出头文件；头文件是C语言中的一个重要概念，它可以是一些函数的集合，这些函数可以被我们直接调用，从而实现我们想要的功能。
...
/**
	第一个C语言程序
		1."main"函数
			a."main"函数是程序的入口。
			b."main"函数可以在文件中的任意位置。
			c.在一个工程中"main"函数有且仅有一个。
		2."printf"函数可以在屏幕上打印(输出)信息，在使用库函数时需要引用其对应的头文件。
		3.C语言的每一条语句都是以分号结尾的。
*/
...
//{
//	printf("打印的第一串代码\n");//"printf"：库函数，意为在屏幕上打印(输出)信息；"printf"函数的使用需要引用头文件"stdio.h"。
//	printf("打印的第二串代码\n");//"\n"：转义字符，代表换行。
//	printf("打印的第三串代码\n");//C语言的每一条语句都是以分号结尾的。
//	return 0;
//}
...
/**
	C语言的基本数据类型：为了更加丰富的表达生活中的各种值，就有了不同的数据类型。
		1.整型家族
			...
			c."int"：整型，系统给定的基本整数类型，C语言规定"int"类型的大小至少为两个字节，通常以计算机的自然字长而定。
...
//{
//	//整型家族的大小
//	printf("%zu\n", sizeof(char));            //打印字符数据类型的大小；"%zu"：属于转换格式指示符(格式化数据)，意为以无符号整型的形式(格式)打印。
...
/**
	常量和变量
		...
		6.字符串：即用双引号引起来的一串字符。
			...
			c.使用"char"字符数据类型及其相关变体，可定义字符、字符串以及字符数组。
*/
...
//
//	double weight = 60.5;//双精度变量不需要添加"f"，默认不添加代表双精度浮点型(double)。
//
//	age = age + 1;//改变一个变量。
//
...
//
//	scanf_s("%d%d", &num1, &num2);//"scanf_s"：输入函数，类似于"scanf"输入函数，但在"VS2022"中不做屏蔽警告处理无法使用"scanf"函数，因其不安全。
//
//	sum = num1 + num2;
//
//	printf("%d + %d = %d\n", num1, num2, sum);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//注释：注释是用于解释代码的。
//	1.C语言的注释风格为：" /*...*/ "，不支持嵌套注释，即遇到第一个"*/"就代表注释结束了。(知道我这一段为什么用" // "了吧...)
//	2.C++语言的注释风格为：" // "，C语言于C99标准中将其引入。
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

/**
	选择语句：又称为分支语句。
		1.具有相关的"if"关键字、"else"关键字和"else if"关键字。
...
/**
	循环语句：可以循环执行的语句(代码段)。
		1.具有相关的"while"循环、"for"循环和"do-while"循环。
*/
...
//	printf("%d\n", a);//打印的结果为：4。
//
//	float b = 9.0 / 2;//两端需要取一个为小数，答案才能为"4.500000"，因当两端为整数，两整数运算结果还是整数。
//	printf("%f\n", b);//打印的结果为：4.500000。
//
//	int c = 9 % 2;//"%"：取模(余数)操作符。
//	printf("%d\n", c);//打印的结果为：1。
...
//	int max = 0;
//
//	max = a > b ? a : b;//如果"a > b"成立，则结果为"a"，否则为"b"，然后再将值赋给"max"变量。
//
//	printf("%d\n", max);
//
//	return 0;
...
/**
	关键字：由C语言提供的，具有特殊含义的单词，不能用作变量名或者函数名。
		...
		2.C语言的关键词数量视标准而定。
		3.C语言的关键词总览：https://zh.cppreference.com/w/c/keyword。
*/
...
//		int b = 0;//省略了"auto"关键词。
//	}
//
//	//register关键字：大量或频繁被使用的数据，放在寄存器中可提升效率。
//	{
//		register int a = 0;//建议变量"num"的值存放在寄存器中(是否存放取决于编译器)，寄存器存在于存储类元件的速度金字塔顶尖。
//	}
//
//	//"define"和"include"不是关键字，是预处理指令。
//	//"define"可定义标识符常量，也可定义宏。
//	{
...
//	{
//		typedef unsigned int u_int;//将"unsigned int"起了个别名叫"u_int"，意义在于缩短了名字。
//		unsigned int a = 100;//"unsigned"：意为无符号的。
//		u_int b = 100;//可以发现同时起作用。
//	}
//
//	//存储的区域可大致分为栈区(局部变量和函数参数)、堆区(动态内存分配的)和静态区(全局变量、静态变量、由"static"修饰的变量)等。
...
//	printf("%d\n", a);//打印的结果为：10。
//
//	int* pa = &a;//变量"pa"是用来存放地址的，在C语言中"pa"叫指针变量，"*"说明"pa"其是指针变量，而"int"说明"pa"指向的对象是"int"类型的，而此处的"&"操作符意为取出变量"a"的地址(取出的是变量"a"第一个字节的地址)。
//	*pa = 20; //"*"：间接访问操作符，又称解引用操作符，此处的"*pa"就是通过"pa"里边的地址，找到变量"a"，并将变量"a"赋值为"20"。
//	printf("%d\n", a);//打印的结果为：20。
//
//	return 0;
//}
