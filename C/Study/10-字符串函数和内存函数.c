...
//{
//	while (*str1 != '\0' && (*(unsigned char*)str1 - *(unsigned char*)str2 == 0))
//	{
//		str1++;
//		str2++;
//	}
...
//{
//    while (count-- && (*(const unsigned char*)str1 - *(const unsigned char*)str2) == 0 && *str1 != '\0')
//    {
//        str1++;
//        str2++;
//    }
	...
	"strtok"函数：以标记拆分字符串。
		...
		4.函数的返回值设计
			a.若"str"不是空指针，则调用被当做"strtok"对此特定字符串的首次调用，函数搜索首个不含于"delimiters"的字符。
				1.若找不到这种字符，则"str"中完全没有记号，而函数返回空指针。
				2.若找到这种字符，则它是记号的起始。然后函数从该点搜索首个含于"delimiters"的字符。
...
/*
	"memcpy"函数：复制内存块。(C标准库要求此函数只需实现复制不重叠内存块即可，但于"VS2022"中"memcpy"函数同时实现了可复制重叠内存块功能)
		1.头文件："string.h"。
...
//	//const unsigned char* str2 = (const unsigned char*)source;
//	//while (num--)
//	//{
//	//	*str1++ = *str2++;
//	//}
...
//	while (num--)
//	{
//		*((unsigned char*)(tmp = (unsigned char*)tmp + 1) - 1) = *((const unsigned char*)(source = (const unsigned char*)source + 1) - 1);
//	}
//
//	return destination;
//}
...
/*
	"memmove"函数：移动内存块。
		...
		3.函数说明：
			...
			c.该函数不检查源中的任何终止空字符，它将直接精确复制"num"个字节数据。
			d.可用于设置分配函数所获得对象的有效类型。
			e.区别于"memcpy"函数，"memmove"允许目标空间和源空间重叠，但不如"memcpy"高效。
			f.尽管说明了"如同"使用临时缓冲区，但此函数的实际实现不会带来二次复制或额外内存的开销。
			g.在严格别名禁止检验同一内存作为两种不同类型的值的情况下，可用"memmove"函数转换值。
			h.若目标在源之前开始，则从缓冲区开始正向复制，否则从末尾反向复制，完全无重叠时回落到更高效的"memcpy"。
		4.未定义情况
			a.若访问发生在"destination"数组结尾后，则行为未定义。
...
//		while (num--)
//		{
//			*((unsigned char*)(ptr = (unsigned char*)ptr + 1) - 1) = *((const unsigned char*)(source = (const unsigned char*)source + 1) - 1);
//		}
//	}
//	else
//	{
//		while (num--)
//		{
//			*((unsigned char*)ptr + num) = *((unsigned char*)source + num);
//		}
//	}
//
//	return destination;
...
//{
//	int arr[10] = { 5,6,7,8,9,0,1,2,3,4 };
//	My_Memmove(arr, arr + 5, sizeof(arr[0]) * 5);
//	for (int i = 0; i < sizeof(arr)/sizeof(arr[0]); i++)
//	{
//		printf("%d ", arr[i]);
//	}
...
/*
	"memset"函数：填充内存块。
		...
		3.函数说明：将值"value"(类型转换为"unsigned char")复制到"ptr"所指向对象的最前面"num"个字节中。
		4.未定义情况
