...
/*
	动态内存管理
		1.动态内存分配：在程序执行的过程中动态地分配或者回收存储空间的分配内存的方法。
			a.动态内存分配不像数组等静态内存分配方法那样需要预先分配存储空间，而是由系统根据程序的需要即时分配，且分配的大小就是程序要求的大小。
			b.动态内存开辟与堆上。
		2.动态内存函数的介绍："malloc"函数、"calloc"函数、"realloc"函数、"free"函数。
		3.常见的动态内存错误
			a.对空指针的解引用操作
			b.对动态开辟空间的越界访问
			c.对非动态开辟内存使用"free"函数释放
			d.使用"free"函数释放一块动态开辟内存的一部分
			e.对同一块动态内存多次释放
			f.动态开辟内存忘记释放(内存泄漏)
		4.笔试题
		5.C/C++程序的内存开辟
...
//	//If_Error(ptr);
//	////Init_For_Zero(num, ptr);//"calloc"函数会默认初始化所有分配存储中的字节为零。
//	//Point(num, ptr);
//	//Free(ptr);
//
//	////"realloc"函数使用示例
//	//ptr = (int*)calloc(num, sizeof(int));//先使用"calloc"函数分配内存块并初始化。
//	//If_Error(ptr);
//	//Point(num, ptr);
//	//num = 20;
//	//int *ptmp = (int*)realloc(ptr, num * sizeof(int));//使用"realloc"函数调整内存空间为"num"个"int"的大小；使用临时指针接收"realloc"，防止错误。
//	//if (ptmp != NULL)
//	//{
//	//	ptr = ptmp;
//	//}
