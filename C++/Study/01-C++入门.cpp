...
#include <stdlib.h> //C语言中的"standard library"标准库函数头文件，不包含命名空间。
...
/**
	C与C++
		1.C语言是结构化和模块化的语言，适合较小规模的程序，但对于复杂的问题，以及规模较大的程序需要高度的抽象和建模时，使用C语言并不合适。
...
//using std::endl;//使用"using"关键词，使"endl"函数可用。
//
//int main()
//{
//	cout << "Hello World!" << endl;
...
/**
	C++关键词
		1.C语言约有"32"个关键词(视标准)，而C++的每套标准关键词数量都可能不同：C++11前共有"63"个关键词，而C++11起共有"73"个关键词，往后标准的关键词数量变化见此段的"3.C++的关键词总览"。
		2.各个关键词的细节将包含在各个章节中。
...
/**
	命名空间：又称为名称空间，"std"是C++官方标准库的命名空间。
		...
		07."using"关键词C++提供了两个机制："using"声明使特定的标识符可用和"using"编译指令使整个命名空间可用(结合"namespace"关键词)。
		08.早期具有".h"后缀的头文件中是不支持名称空间的，并且C语言相关是不支持名称空间的，支持名称空间的C语言头文件是将".h"去除并在文件名最前方加上"c"(融入C++)。
		...
		10.域
			a.实际上名称空间也属于域，但其其中所包含的各种名称是相当于在全局域中的，并可以理解成把这些名称包含在了一个空间内。
			...
			g.局部变量是存放在内存中的栈上的，而名称空间内所定义或声明的变量是和全局变量一样存放在内存中的数据段内的。
*/
...
//int main()
//{
//	printf("%p\n", rand);//此处打印的是"stdlib.h"中的"rand"函数地址，若将全局变量"rand"的注释解除后再编译，会有声明与已在范围内使用声明的目标冲突的问题。
//	return 0;
//}
...
//int main()
//{
//	struct N1::Node Node1 = { 0, nullptr };
//	struct N1::N2::Node Node2 = { 0, nullptr, nullptr };
//
...
//
//	printf("%d\n", rand);//采用默认搜索规则，直接在局部域中找到了标识符为"rand"的变量，故此处结果为：5。
//
...
/**
	C++的输入和输出
		...
		2."cout"和"cin"都是全局的流对象，而"endl"是特殊的C++符号(函数模板)，表示换行输出(也可以使用转义字符)，它们都包含在"iostream"头文件中。
		3."<<"为流插入运算符，">>"是流提取运算符。
		4.使用"cout"或"cin"进行输出或输入操作，可以自动识别变量类型，并不需要像"printf"函数或"scanf"函数那样，需要手动指定格式化数据。
...
/**
	缺省参数：在声明或定义函数某个参数的时候为之指定一个默认值，在调用该函数的时候如果采用该默认值，则无须指定该参数，即为缺省。
		1.全缺省：即声明或定义函数时，为每一个参数都指定了一个默认值。
...
//int main()
//{
//	Func();//不传递任何实参给形参，并且"Func"函数是全缺省的，此时"Func"函数的参数的值全部为其默认值。
//	Func(1);//只将值"1"传递给了"Func"函数的形参"a"变量，此时函数的其他变量依旧是默认值。
//	Func(1, 2);//将值"1"和值"2"传递给了"Func"函数的形参"a"变量和形参"b"变量，此时函数的其他变量依旧是默认值。
...
/**
	函数重载：C++允许在同一个作用域中声明几个功能类似的同名函数，这些函数的形参列表不同，常用来处理实现功能类似，但数据类型不同的问题。
		1.函数重载，代表函数同名，并且因为形参列表不同而实现重载，但是函数不能基于返回值类型不同实现重载。
			a.形参的类型不同。
			b.形参的个数不同。
...
/**
	引用
		1.引用概念
			...
			g.常规变量和"const"变量都可视为左值，但常规变量可被修改，而"const"变量不可直接被修改。
		2.引用特性
			a.在声明引用变量时必须进行初始化。
			b.一个变量可以有多个引用。
			c.引用变量一旦与一个实体关联起来，就无法再改变，即无法再引用其他实体。
		3.常引用：在引用变量前加上"const"关键词，即可创建常引用，表示不能通过"const"的引用修改被引用的变量。
		...
		4.使用场景
			a.作为函数参数传递
				...
				b'.如果实参与引用参数不匹配，C++将生成临时变量。(当且仅当参数为"const"引用时，C++才允许这样做)
				c'.在引用形参中，应尽量使用"const"。
					a''.可以避免无意中修改了数据导致的错误。
					b''.使函数能够处理"const"和非"const"实参，否则只能处理非"const"实参。
					c''.使参数传递过程中能够正确生成临时变量，并使函数能正确使用临时变量。
			b.作为函数返回值
		...
		6.引用和指针的区别
			a.语法概念和底层实现上的区别
				...
				b'.引用在底层实现上实际是有空间的，因为引用是按照指针方式来实现的(反编译后对比代码)。(使实现而定)
				c'.引用不是对象，它们不必占用存储，尽管编译器会在需要实现所需语义的情况下分配存储(例如：引用类型的非静态数据成员通常会增加类的大小，量为存储内存地址所需)。
			b.引用概念上定义一个变量的别名，而指针存储一个变量地址。
			c.引用必须初始化(引用必须被初始化为指代一个有效的对象或函数)，而指针可以不初始化。
			...
			h.有多级指针的概念，但是没有多级引用的概念，即不存在引用的引用。
...
//
//	std::cout << "cpi1 = " << cpi1 << std::endl;//此时打印的是临时变量的值。
//
...
//int& Count2()
//{
//	static int n = 0;//静态变量，存放于内存中的数据段，"Count2"函数执行完毕后，静态变量的生命周期不会结束，"n"的值会一直保留。
//	std::cout << "&n = " << &n << std::endl;
//	return ++n;
//}
//
...
//	Func2(a);//权限缩小，传入函数"Func2"的是变量"a"的常引用，函数"Func2"不能直接对常引用形参"n"进行修改，也代表函数"Func2"不能直接对变量"a"进行修改。
//	Func2(ra);//权限缩小，传入函数"Func2"的是变量"ra"的常引用，而变量"ra"实际上是变量"a"的别名，那么函数"Func2"中的形参"n"也是变量"a"的常引用，函数"Func2"不能直接对常引用形参"n"进行修改，也代表函数"Func2"不能直接对变量"a"进行修改。
//
//	const int b = 0;
...
//	int a = 10;
//
//	//以下代码反汇编后可以发现：引用在底层实现上，实际是占据空间的(寄存器)，因为引用是按照指针方式来实现的。(使实现而定)
//	int& ra = a;
//	ra = 20;
//	int* pa = &a;
//	*pa = 20;
...
/**
	"auto"关键词(C++11)
		...
		4.新鲜的"auto"关键词(拓展)
			...
			b.自C++17起，对于非类型模板形参，指定要从它的模板实参推导出它的类型，而对于函数形参，指定要从它的实参推导出它的类型。
*/
...
//	//auto int y;//在C++11前合法，C++11起不合法，部分编译器报警告："auto"存储类说明符在C++11中是不允许的，并且在未来的版本中将不被支持。
//
//	//C++11起的"auto"关键词
