...
/***
	运算符重载：即为用户定义类型的操作数定制C++运算符，其将重载的概念扩展到运算符上，允许赋予C++运算符多重含义。
		...
		4.将运算符重载为类的成员函数或非成员函数。
			...
			b.作为非成员函数重载时，若要访问类中的私有成员，需要将其声明为友元函数。(友元函数将会在本章的第"15"小节中介绍)
	...
	流提取与流插入运算符重载
		...
		3.重载">>"流提取运算符和"<<"流插入运算符，使其支持用户自定义类型。
			...
			d.当把流提取与流插入运算符重载函数定义在头文件中时(这也不仅仅是此函数的问题，只要是在头文件中定义的全局变量或全局函数都会出现此问题)，可能会出现重复定义的错误，因为头文件在多处源文件中包含，导致链接错误。
				...
				b.第二种非主流解决方法：由C语言继承而来，加上"static"关键字，使得此函数或变量只在当前源文件中可见，而不会在其他源文件中可见，即将外部链接改为内部链接，并且其具有静态或线程存储期。
				c.第三种小妙招解决方法：加上"inline"关键字，使带外部连接的"inline"函数或变量(C++17起才具有内联变量)(例如不声明为 "static")拥有额外属性。
					a'.使得此函数或变量在程序中可以有多于一次定义，只要每个定义都出现在不同翻译单元中(对于非静态的内联函数和变量(C++17起)且所有定义等同即可。
					b'.它必须在每个翻译单元中都被声明为"inline"。
					c'.它在每个翻译单元中都拥有相同的地址。
				d.第四种不常用解决方法：使用匿名命名空间解决，类似于"static"关键字的解决方案，即将全局变量或全局函数放在匿名命名空间中，使得每个文件都有自己的一份定义，而不会出现重复定义的错误。
...
//	}
//
//	//友元函数的声明：注意此处的友元声明并不是代表着此函数是此类的成员函数，而是代表着此函数可以访问此类的私有成员，将在本章后面的小节进行详细介绍。
...
//	}
//
//	//通过"const_cast"关键字绕过具有"const"限定符的成员函数无法调用没有"const"限定符的成员函数的限制
//	void f1(int v) const
//	{
